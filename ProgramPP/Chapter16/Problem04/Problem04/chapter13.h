...
struct Regular_polygons : Shape {
	...
	Regular_polygons(Point c, int dd, int nn);
...
struct Right_triangle : Closed_polyline {
	...
	Right_triangle(Point rr, int ww, int hh);	
...
struct Poly : Closed_polyline {
	...
	Poly(vector_ref<Point> pts);

	void add_points();
	void draw_lines() const;

private:
	vector_ref<Point> pts;
};

//------------------------------------------------------------------------------

struct Star : Closed_polyline { // these are known as isotaxal stars so lines don't intersect.

	Star(Point c, int npp, int rr);	
