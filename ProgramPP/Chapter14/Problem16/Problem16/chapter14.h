...
public:
	...
	virtual void set_level(int level) { l = level; }
...
struct Controller_test : public Controller {

	void show() const; // prints the current state and level
};


struct Controller_shape : public Controller {
	
	Controller_shape(Shape& s) : shape{ s } {}

	void on(); // on/of determine if the shape is visible or invisible
	void off();
	void control_level(int level); // level determines the line thickness
	void control_color(Color c);
