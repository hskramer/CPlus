...
namespace Roman {
	...
	{
		switch (ch) {
		case N: case I: case V: case X: case L: case C: case D: case M:
			return true;
		default:
			return false;
		}
	...
	{
		// values should decrease from left to right with the exception of repeating C's between M's still catches more then 3 
		int left{ 0 }, right{ 0 };
		bool is_valid{ false };
	...
	{
		switch (r) { // the Roman's had no numeral for zero they did have a word nulla for nothing (Wikipedia) and about 725AD some used N for nulla
		case N:  return 0;
		case I:	 return 1;
		case V:	 return 5;
		case X:	 return 10;
	...
	Roman_int operator-(const Roman_int& r1)
	{
		return -r1.as_int();
	}
